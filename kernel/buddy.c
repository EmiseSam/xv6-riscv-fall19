...
// Converse bit at position index in array
...
void bd_free(void *p)
{
  ...
  for (k = size(p); k < MAXSIZE; k++)
  {
    ...
    // }
    if (bit_isset(bd_sizes[k].alloc, buddy >> 1))
    {        // is buddy allocated?
      break; // break out of loop
    }
...
// Return 1 if a block is within the free zone
int addr_in_range(void *blockStart, void *blockEnd, void *freeStart, void *freeEnd)
{
  return (blockStart >= freeStart && blockEnd < freeEnd);
}
...
int bd_initfree_pair(int k, int bi, void *left, void *right)
{
  ...
  {
    free = BLK_SIZE(k);
    if (addr_in_range(addr(k, bi), addr(k, bi) + BLK_SIZE(k) - 1, left, right))
